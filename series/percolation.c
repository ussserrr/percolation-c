...
void find_clusters(Grid *g_ptr) {
	int i, j, k, y, x, id_of_first_neighbor, id_of_second_neighbor, np_cnt, wrong_id, correct_id;
	...
	do {
		...
		for (i=0; i<g_ptr->num_of_ones; i++) {
			...
			if (g_ptr->grid[y-1][x]==1 && g_ptr->grid[y][x-1]==0) {
				...
				g_ptr->ids[i] = (id_of_first_neighbor<id_of_second_neighbor) ? id_of_first_neighbor : id_of_second_neighbor;
				...
				if (id_of_first_neighbor!=id_of_second_neighbor) {
					...
					for (j=0; j<g_ptr->num_of_ones; j++)
						if (g_ptr->ids[j]==wrong_id)
							g_ptr->ids[j] = correct_id;
				}
			}
		}
...
PercolationType is_percolation(Grid *g_ptr) {
	...
	bool upwards = false,\
		 lefttoright = false;
	int i, k, y, x, size_of_perc_cluster, len_of_uniq;
	...
	for (i=0; i<len_of_uniq; i++) {
		...
		for (k=0; k<g_ptr->num_of_ones; k++) {
			if (g_ptr->ids[k]==uniq_clusters[i]) {
				...
				size_of_perc_cluster++;
			}
		}

		if (y_eq_1 && y_eq_GRID_Y_DIM) {
			upwards = true;
			g_ptr->num_of_clusters_with_perc++;
			g_ptr->clusters_with_perc = realloc(g_ptr->clusters_with_perc, g_ptr->num_of_clusters_with_perc*sizeof(int));
			g_ptr->clusters_with_perc[g_ptr->num_of_clusters_with_perc-1] = uniq_clusters[i];
			g_ptr->lens_of_clusters_with_perc = realloc(g_ptr->lens_of_clusters_with_perc, g_ptr->num_of_clusters_with_perc*sizeof(int));
			g_ptr->lens_of_clusters_with_perc[g_ptr->num_of_clusters_with_perc-1] = size_of_perc_cluster;
		}
...
// PercolationType single_exprmnt_optimized(int y_dim, int x_dim, double probability_of_1) {
// }
