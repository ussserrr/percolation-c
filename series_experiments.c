...
int main(void) {
	...
	for (int grid_size_i=0; grid_size_i<N_GRID_DIMS; grid_size_i++) {
		...
		// For each grid size we create a file with a name consisting of the size
		// and the number of experiments
		char fname[20];
		snprintf(fname, 20, "%dx%d, N=%d", grid_dim[grid_size_i], grid_dim[grid_size_i], N_EXPER);
		...
		for (int probab_i=0; probab_i<N_PROBAB; probab_i++) {
			...
			for (int exper_i=0; exper_i<N_EXPER; exper_i++) {
				result = single_exprmnt(g_ptr, grid_dim[grid_size_i], grid_dim[grid_size_i], probab);
			...
			fprintf(fp, "%1.2f\t%1.2f\n", probab, (double)num_of_exper_ended_with_percolation/N_EXPER);
